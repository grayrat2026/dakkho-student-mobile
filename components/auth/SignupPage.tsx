...
export function SignupPage() {
  ...
  // If needs verification, jump to OTP step
  React.useEffect(() => {
  ...
  const handleVerifyOTP = () => {
    const code = otp.join('');
    if (code.length === 6) {
      // Would call authApi.verifyOTP(email, code)
    ...
    <View style={styles.stepRow}>
      {STEPS.map((s, i) => (
        <View key={s} style={styles.stepItem}>
          <View style={[
            styles.stepCircle,
            i <= step ? { backgroundColor: colors.primary } : { backgroundColor: colors.muted },
          ]}>
                ...
                <MotiView from={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
                  ...
                  <View style={styles.otpRow}>
                    {otp.map((digit, i) => (
                      <TextInput
                        key={i}
                        style={[styles.otpInput, { borderColor: digit ? colors.primary : colors.border, color: colors.foreground }]}
                        maxLength={1}
...
const styles = StyleSheet.create({
  ...
  stepRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', marginBottom: 24, gap: 4 },
  stepItem: { alignItems: 'center' },
  stepCircle: { width: 28, height: 28, borderRadius: 14, alignItems: 'center', justifyContent: 'center' },
  stepNum: { color: '#fff', fontSize: 12, fontWeight: '700' },
  stepLabel: { fontSize: 9, fontWeight: '600', marginTop: 4 },
  stepLine: { width: 20, height: 2 },
  errorText: { color: '#ef4444', fontSize: 12, fontWeight: '500', marginBottom: 12, textAlign: 'center' },
  ...
  otpSubtitle: { fontSize: 14, textAlign: 'center', marginBottom: 16 },
  otpRow: { flexDirection: 'row', justifyContent: 'center', gap: 8, marginBottom: 16 },
  otpInput: {
    width: 44, height: 52, borderRadius: 12, borderWidth: 2, textAlign: 'center',
    fontSize: 20, fontWeight: '700',
  },
  resendText: { fontSize: 13, fontWeight: '600', textAlign: 'center', marginBottom: 16 },
  actionBtn: { borderRadius: 12, overflow: 'hidden', marginTop: 8 },
